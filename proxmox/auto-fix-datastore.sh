#!/bin/bash
################################################################################
# WeatherStation Auto Datastore Fix Script
# Run this script from the Proxmox HOST - no prompts needed
################################################################################
...
echo "🔧 WeatherStation Auto Datastore Fix Tool"
echo "========================================="
echo ""

# Auto-detect WeatherStation containers
print_info "Auto-detecting WeatherStation containers..."
WEATHER_CONTAINERS=$(pct list | grep "running" | awk '{print $1}' | while read id; do
    if pct exec $id -- systemctl is-active weatherstation > /dev/null 2>&1; then
        echo $id
    fi
done)

if [ -z "$WEATHER_CONTAINERS" ]; then
    print_error "No running WeatherStation containers found"
    print_info "Please run this script from your Proxmox host"
    exit 1
fi

# Use the first found container
CONTAINER_ID=$(echo "$WEATHER_CONTAINERS" | head -1)
print_info "Found WeatherStation container: $CONTAINER_ID"

# Get container IP
CONTAINER_IP=$(pct exec $CONTAINER_ID -- ip addr show eth0 | grep 'inet ' | awk '{print $2}' | cut -d/ -f1)
print_info "Container IP: $CONTAINER_IP"

print_info "Fixing datastore structure..."
