#!/bin/bash
################################################################################
# WeatherStation - Fresh Installation Script for Proxmox v6
...
#
# Usage: ./fresh-install-v6.sh
# Or: bash -c "$(curl -fsSL https://raw.githubusercontent.com/crowninternet/WeatherStation/main/proxmox/fresh-install-v6.sh)"
################################################################################
...
print_info "Creating API configuration..."
pct exec $CONTAINER_ID -- bash -c "cat > $INSTALL_DIR/.env << 'EOF'
# AmbientWeather API Configuration
AMBIENT_API_KEY=5f330743134b4c4aa38e7c98dd98c3f7f75dfe5912e8447089acc7c780f649e1
AMBIENT_APP_KEY=0f6e52a372084f6a9b92673d2d32a2d1a54372ace9b3440dae3b44e72c48a44a
...
echo ""
print_info "Triggering initial data ingestion..."
sleep 5  # Wait for service to fully start
pct exec $CONTAINER_ID -- bash -c "curl -s 'http://localhost:3333/tasks/ingest?token=W8*q#G\$4pKR058oL' > /dev/null || true"
print_success "Initial data ingestion triggered"
...
echo " 2. Your API credentials are pre-configured!"
echo " 3. Initial data ingestion has been triggered"
echo " 4. Your weather stations should be visible immediately"
echo " 5. Enjoy your weather dashboard!"
echo ""
echo "✅ API credentials are pre-configured from your localhost setup"
echo "✅ Initial data ingestion has been triggered automatically"
echo "📖 See proxmox/TROUBLESHOOTING.md if you need help"
echo ""
